...
function TermsOfConditionsScreen({ navigation }) {
    ...
    const onDonePress = () => {
        if (!agree) {
            return presentToastMessage({ type: 'success', position: 'top', message: "Please confirm your agreement to our terms & conditions." })
        }
        saveTermsAccepted()
    }
    const saveTermsAccepted = async () => {
        try {
            let parameters = {
                'is_terms_accepted': 1
            }
...
export default TermsOfConditionsScreen;