...
function MyPreferencesScreen({ navigation }) {
    ...
    const onNextPress = () => {
        ...
        if (hopeToFind.length === 0) {
            return presentToastMessage({ type: 'success', position: 'top', message: "Please select the types of person you are hoping to find." })
        }
        if (ideaOfFun.length === 0) {
            return presentToastMessage({ type: 'success', position: 'top', message: "Please select the types of your idea of fun date." })
        }
        if (bodyType.length === 0) {
            return presentToastMessage({ type: 'success', position: 'top', message: "Please select the body types you prefer." })
        }
        if (smokeType.length === 0) {
            return presentToastMessage({ type: 'success', position: 'top', message: "Please select the smoking habits you prefer." })
        }
        if (drinkType.length === 0) {
            return presentToastMessage({ type: 'success', position: 'top', message: "Please select the drinking habits you prefer." })
        }
        if (educationLevel.length === 0) {
            return presentToastMessage({ type: 'success', position: 'top', message: "Please select the education levels you prefer." })
        }
        if (politicalPreference.length === 0) {
            return presentToastMessage({ type: 'success', position: 'top', message: "Please select the political preferences you prefer." })
        }
        if (culturalBackground.length === 0) {
            return presentToastMessage({ type: 'success', position: 'top', message: "Please select the ethnicities/cultural backgrounds you prefer." })
        }
        savePreferences()
    }
                ...
                <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 30, paddingBottom: 20, alignItems: 'center' }}>
                    ...
                    <View style={{ marginTop: 24, width: Constants.LAYOUT.SCREEN_WIDTH - 60 }}>
                        ...
                        <StyledDropdown
                            ...
                            options={Constants.PREFERENCE_POLITICAL_OPTIONS}
